...
class Crud_model extends CI_Model
{
    ...
    public function update_mtn_settings()
    {
        // update paypal keys
        $mtn_info = array();
        $mtn['active'] = $this->input->post('mtn_active');
        $mtn['mode'] = $this->input->post('mtn_mode');

        $mtn['sandbox_mtn_user_id'] = $this->input->post('sandbox_mtn_user_id');
        $mtn['sandbox_mtn_api_key'] = $this->input->post('sandbox_mtn_api_key');
        $mtn['sandbox_mtn_subscription_key'] = $this->input->post('sandbox_mtn_subscription_key');

       $mtn['production_mtn_user_id'] = $this->input->post('production_mtn_user_id');
        $mtn['production_mtn_api_key'] = $this->input->post('production_mtn_api_key');
        $mtn['production_mtn_subscription_key'] = $this->input->post('production_mtn_subscription_key');



        array_push($mtn_info, $mtn);

        $data['value']    =   json_encode($mtn_info);
        $this->db->where('key', 'mtn');
        $this->db->update('settings', $data);

        $data['value'] = html_escape($this->input->post('mtn_currency'));
        $this->db->where('key', 'mtn_currency');
        $this->db->update('settings', $data);
    }


    public function update_fedapay_settings()
    {
        // update paypal keys
        $fedapay_info = array();
        $fedapay['active'] = $this->input->post('fedapay_active');
        $fedapay['mode'] = $this->input->post('fedapay_mode');

        $fedapay['sandbox_fedapay_private_key'] = $this->input->post('sandbox_fedapay_private_key');
        $fedapay['sandbox_fedapay_public_key'] = $this->input->post('sandbox_fedapay_public_key');

        $fedapay['prod_fedapay_private_key'] = $this->input->post('prod_fedapay_private_key');
        $fedapay['prod_fedapay_public_key'] = $this->input->post('prod_fedapay_public_key');


        array_push(  $fedapay_info, $fedapay);

        $data['value']    =   json_encode(  $fedapay_info);
        $this->db->where('key', 'fedapay');
        $this->db->update('settings', $data);
    ...
    public function update_paydunya_settings()
    {
        // update paypal keys
        $paydunya_info = array();
        $paydunya['active'] = $this->input->post('paydunya_active');
        $paydunya['mode'] = $this->input->post('paydunya_mode');
        $paydunya['MasterKey'] = $this->input->post('MasterKey');

        $paydunya['sandbox_PublicKey'] = $this->input->post('sandbox_PublicKey');
        $paydunya['sandbox_PrivateKey'] = $this->input->post('sandbox_PrivateKey');
        $paydunya['sandbox_Token'] = $this->input->post('sandbox_Token');

        $paydunya['production_PublicKey'] = $this->input->post('production_PublicKey');
        $paydunya['production_PrivateKey'] = $this->input->post('production_PrivateKey');
        $paydunya['production_Token'] = $this->input->post('production_Token');



        array_push($paydunya_info, $paydunya);

        $data['value']    =   json_encode($paydunya_info);
        $this->db->where('key', 'paydunya');
        $this->db->update('settings', $data);
...
//        $this->db->update('settings', $data);
    ...
    public function course_purchase($user_id, $method, $amount_paid, $param1 = "", $param2 = "")
    {
        ...
        $transaction_info = $this->session->userdata('transaction_info');

//        var_dump($transaction_info['commission']);
//        die();
        ...
        foreach ($purchased_courses as $purchased_course) {
            ...
            if (get_user_role('role_id', $course_details['creator']) == 1) {
                ...
                // modification pour les payements des formateurs
                ...
                if (get_settings('allow_instructor') == 1) {
                    ...
                    if($method=="fedapay"){
                        $data['amount'] = $data['amount'] -$transaction_info['fees'];

                    }

                    if($method=="mtn"){
                        $data['amount'] = $data['amount'] - ($data['amount'] * 0.015);
            ...
            $this->session->set_userdata('transaction_info',  array());
        }
